...
class MarkovGeneratorDE(object):
  ...
  def __init__(self, n, max, min=5):
    ...
    self.names = set()
    for line in open("data/namesAll.txt"):
    	self.names.add(line.strip())
    self.nouns = set()
    for line in open("data/listNouns.txt"):
    ...
    self.bt = {}
    for line in open('data/brownList.txt'):
    	words = line.split()
    	tag = words[0]
    	self.bt[tag] = set()
    	for word in words[1:]:
  ...
  #Tag all words in scanned article by parts of speech
  ...
  def makeLocationsArray(self,line):
      ...
      for word in line.split():
          if word in self.articleWordPositions:
              ...
              #some english-specific article thing happening here, ignore for German
              """
